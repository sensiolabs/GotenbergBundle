...
//                ],
        ...
        foreach ($list as $builderType => $builder) {
            foreach ($builder as $builderName => $expectedConfig) {
                $configuratorDefinition = $containerBuilder->getDefinition(".sensiolabs_gotenberg.{$builderType}_builder_configurator.{$builderName}");

                $config = $configuratorDefinition->getArguments();
